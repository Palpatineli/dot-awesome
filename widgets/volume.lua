...
local volume = wibox.container.mirror(volumearc, { horizontal = true })

local update_graphic = function(widget, stdout, _, _, _)
    local mute = string.match(stdout, "%[(o%D%D?)%]")
    local volume_value = tonumber(string.format("% 3d", string.match(stdout, "(%d?%d?%d)%%")))

    widget.value = volume_value / 100;
    if mute == "off" then
